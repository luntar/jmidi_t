...
{
    MidiCallbackMessage (MidiInput* i, const MidiMessage& msg) : input(i), message (msg) {}
    MidiMessage message;
    MidiInput* input;
};
...
//        addAndMakeVisible (pairButton);
    ...
    void adjustMidiStringBuffer()
    {
      int idx = 0;
      while (((midiMonitor.getHeight()+20) - midiMonitor.getTextHeight()) < 0)
      {
        idx = _midiStrBuffer.indexOf(idx + 1, "\n");
        if (idx == -1)
          break;
        _midiStrBuffer = _midiStrBuffer.substring(idx).trimStart();
        midiMonitor.setText(_midiStrBuffer, false);
    ...
    void resized() override
    {
        ...
        midiInputSelector->setBounds (margin, 
                                      (2 * margin) + 24,
                                      (getWidth() / 2) - (2 * margin),
                                      hh /*- ((4 * margin) + 24 + 24)*/);
        ...
        incomingMidiLabel.setBounds (margin, (250 + margin)-24, getWidth() - (2 * margin), 24);
...
//                                      getWidth() - (2 * margin), 24);
        ...
        midiMonitor.setBounds (margin, (250+margin),
                               getWidth() - (2 * margin), getHeight() - (250+margin) - margin);
...
// 						}
    ...
    {
        label.setFont (Font ("Hack", 15.00f, Font::plain));
    ...
    Label incomingMidiLabel { "Incoming Midi Label", "Received Raw MIDI Data:" };
